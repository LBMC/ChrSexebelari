...
class FastqSamplerToFasta:
    ...
    def tag_gen(self, sample_number):
        return("/s"+str(sample_number)+"_")

    def file_path_gen(self, R, sample_number):
        tag = self.tag_gen(sample_number)
        file_pair = self.path_leaf(self.fastq_R1)
        if R == 2:
            file_pair = self.path_leaf(self.fastq_R2)
        return(tag + file_pair + str(self.blast_sufix) + ".fasta")

    def test_sampling(self):
        sampling_done = True
        for sample_number in range(self.sample_number):
            if not os.path.isfile(self.output_folder + self.file_path_gen(1, sample_number)):
                sampling_done = False
            else:
                if not os.path.getsize(self.output_folder + self.file_path_gen(1, sample_number)) > 0:
                    sampling_done = False
            if self.paired:
                if not os.path.isfile(self.output_folder + self.file_path_gen(2, sample_number)):
                    sampling_done = False
                else:
                    if not os.path.getsize(self.output_folder + self.file_path_gen(2, sample_number)) > 0:
                        sampling_done = False
        if sampling_done:
    ...
    def record_files(self):
        self.files = list()
        for sample_number in range(self.sample_number):
            self.files.append(self.file_path_gen(1, sample_number)[1:])
        if self.paired:
            for i in range(self.sample_number):
                self.files.append(self.file_path_gen(2, sample_number)[1:])

    def add_sufix(self, blast):
        if blast:
            self.blast_sufix = "_blast"
            print("generating blast sample...")
        else:
            print("generating trinity samples...")
        self.R1_gz = False
        self.R2_gz = False
    ...
    def sampling_files(self):
        if not self.test_sampling():
            ...
            for sample_number in range(self.sample_number):
                tag = self.tag_gen(sample_number)
                if self.R1_gz:
                    self.sampling_gz(self.fastq_R1, sample_number)
                else:
                    self.sampling(self.fastq_R1, sample_number)
            if self.paired:
                for sample_number in range(self.sample_number):
                    tag = self.tag_gen(sample_number)
                    if self.R2_gz:
                         self.sampling_gz(self.fastq_R2, sample_number)
                    else:
                        self.sampling(self.fastq_R2, sample_number)
        self.record_files()
...
class Blast:
    ...
    def blast2_run(self):
        ...
        if not os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") or (os.path.isfile(self.output_folder+"/blast_out/unmatching_reads1.fasta") and not os.path.getsize(self.output_folder+"/blast_out/unmatching_reads1.fasta") > 0):
            ...
            print("blast2 command:")
            print(blast)
            blastProcess = subprocess.Popen(str(blast), shell=True)
            blastProcess.wait()
            sortblast = "python3 "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/blast_sorter.py --input_dir "+self.output_folder+"/blast_out/int.reads_vs_annoted.blast.out > "+self.output_folder+"/blast_out/s.reads_vs_annoted.blast.out" + " ; rm " +self.output_folder+"/blast_out/int.reads_vs_annoted.blast.out &&"
    ...
    def blast3_run(self):
        ...
        if not os.path.isfile(self.output_folder+"/blast_out/sorted.reads_vs_unannoted.blast.out") or (os.path.isfile(self.output_folder+"/blast_out/sorted.reads_vs_unannoted.blast.out") and not os.path.getsize(self.output_folder+"/blast_out/sorted.reads_vs_unannoted.blast.out") > 0):
            ...
            print("blast3 command:")
            print(blast)
            blastProcess = subprocess.Popen(str(blast), shell=True)
            blastProcess.wait()
            print("Parsing blast3 output...")
            ...
            blastProcess = subprocess.Popen(str(blast), shell=True)
            blastProcess.wait()
            sortblast = "python3 "+os.path.dirname(os.path.realpath(sys.argv[0]))+"/blast_sorter.py --input_dir "+self.output_folder+"/blast_out/int.reads_vs_unannoted.blast.out > "+self.output_folder+"/blast_out/sorted.reads_vs_unannoted.blast.out" + " ; rm " +self.output_folder+"/blast_out/int.reads_vs_annoted.blast.out"
            sortBlastProcess = subprocess.Popen(str(sortblast),shell=True)
            sortBlastProcess.wait()
        else:
