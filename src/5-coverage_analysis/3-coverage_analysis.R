...
cov.female <- fread("results/coverage_analysis/2018-06-12-MRDR5_vs_Hybrid_assembly.sorted.filtered.counts", sep = "\t", h = T, stringsAsFactors = F)
cov.male <- fread("results/coverage_analysis/2018-06-12-MRDR6_vs_Hybrid_assembly.sorted.filtered.counts", sep = "\t", h = T, stringsAsFactors = F)
...
#At bp level
 ...
 counts.genes.bp <- ComputeNormalizedCount(counts.genes.female, counts.genes.male,
    "bp")
  write.table(counts.genes, "results/coverage_analysis/2018-07-25-FC_normalized_coverage_at_bp_within_genes.txt",
    sep = "\t", quote = F, col.names = T, row.names = F)

#Analysis
 gene <-read.table('results/coverage_analysis/2018-07-24-FC_normalized_coverage_at_gene.txt',
sep="\t", header=T)
