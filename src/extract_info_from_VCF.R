...
# size of contigs
size.genome.mbelari <- read.table("data/ReferenceGenomes/2017_09_13_Mbelari.sizes.genome", 
  h = F, sep = "\t", stringsAsFactors = F)
# annotation
gff.mbelari <- read.csv("data/ReferenceGenomes/Mesorhabditis_belari_JU2817_v2.gff3", 
  h = F, sep = "\t", stringsAsFactors = F)
# genes annotation
...
if (do.computation.per.pool) {
  ...
  variants.female <- fread("results/call_var/2017_09_21_MRDR5_trim_Mbelari_mapped_rmdup_rg_realign_indels.vcf", 
    sep = "\t", sep2 = ";", h = T, skip = 19286)
  variants.male <- fread("results/call_var/2017_09_21_MRDR6_trim_Mbelari_mapped_rmdup_rg_realign_indels.vcf", 
    sep = "\t", sep2 = ";", h = T, skip = 19286)
  
  # Get nb of alleles per pool (AN), number of reads 'equal' to ref/number of reads
  # equal to alt at a given position (DP4 field)
  ...
  write.table(variants.female, "results/call_var/MRDR5_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts.txt", 
    sep = "\t", quote = F, row.names = F)
  system("bash src/date.sh MRDR5_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts.txt")
  write.table(variants.male, "results/call_var/MRDR6_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts.txt", 
    sep = "\t", quote = F, row.names = F)
...
##### Open table of variants with counts
tab.variants.female <- fread("results/call_var/2017_09_25_MRDR5_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts.txt", 
  sep = "\t", h = T, stringsAsFactors = F)
tab.variants.male <- fread("results/call_var/2017_09_25_MRDR6_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts.txt", 
  sep = "\t", h = T, stringsAsFactors = F)
...
if(do.summary.raw) {
  for (sexe in c("male", "female")) {
    ...
    all <- foreach(ctg = unique(tmp$CHROM), .combine = rbind) %dopar% {
      ...
      final <- cbind(temp, data.frame(genes = sapply(temp$POS, function(x) paste(temp.gff$V9[which(temp.gff$V4 <= 
        x & temp.gff$V5 >= x)], collapse = "_")), stringsAsFactors = F))
      final
    }
    stopCluster(cl)
    write.table(all, paste("results/call_var/", sexe, "_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts_genic_information.txt", 
      sep = ""), sep = "\t", quote = F, row.names = F)
    system(paste("bash src/date.sh results/call_var/", sexe, "_trim_Mbelari_mapped_rmdup_rg_realign_indels_counts_genic_information.txt", 
      sep = ""))
    ...
    pdf(paste("results/call_var/distance_from_INDEL_to_nearest_variant_", sexe, ".pdf", 
      sep = ""))
    hist(na.omit(min.dist), breaks = 40000, xlim = c(-20, 20), main = paste("distance_from_INDEL_to_nearest_variant ", 
  ...
  for (sexe in c("male", "female")) {
    ...
    summary.raw.variants <- rbind(summary.raw.variants, data.frame(sexe = sexe, nb.SNPs.all = nb.SNPs.all, 
      ...
      nb.median.depth.all = nb.median.depth.all, nb.mean.depth.all = nb.mean.depth.all, 
...
nb.test.male <- sapply(size.genome.mbelari$V1, function(x) length(which(tab.variants.male$CHROM == 
  x & tab.variants.male$is.INDEL == 0)))
density.tests.sexe <- data.frame(contig = size.genome.mbelari$V1, length = size.genome.mbelari$V2, 
  nb.test.female = nb.test.female, nb.test.male = nb.test.male, stringsAsFactors = F)
