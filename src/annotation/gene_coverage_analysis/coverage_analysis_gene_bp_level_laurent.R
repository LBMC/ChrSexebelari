...
source("~/stage_mbelari/src/functions_claire.R")
 ...
 counts.genes.female <- tbl_df(fread("~/stage_mbelari/results/coverage_analysis/2018-07-25-MRDR5_vs_Hybrid_assembly_sort_in_genes_bp.bed", 
  ...
  counts.genes.male <- tbl_df(fread("~/stage_mbelari/results/coverage_analysis/2018-07-25-MRDR6_vs_Hybrid_assembly_sort_in_genes_bp.bed", 
    h = F, sep = "\t", stringsAsFactors = F))
...
pdf("2018-07-25-all_log2_FC.pdf", w = 10, h = 5)
par(mfrow = c(1, 3))
