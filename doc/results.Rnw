...
Manon Grosmaire told us that OP50 E. coli strain was used to feed the worms. Note that bowtie2 was run in default mode: "search for multiple alignments, report the best one".
...
\begin{enumerate}
  ...
  \item \textbf{create index for reference (need to gunzip the fasta before)}:\\
\textcolor{blue}{create\_index.sh} (PSMN)

  \item \textbf{do mapping (file by file)}:\\
  \textcolor{blue}{mapping\_Mbelari\_Ecoli.sh} (PSMN)

  \item \textbf{pick mapped reads for E.coli and M.belari separately}:\\
    \textcolor{blue}{extract\_names\_from\_fasta.sh, generate\_split\_contig\_name.sh, filter\_Mbelari.sh} (last step on PSMN)

  \item \textbf{extract unmapped reads from BAM for E. coli and M.belari}:\\
  \textcolor{blue}{bamtofq\_Mbelari.sh} (PSMN)\\
  \textcolor{blue}{QC_unmapped.sh}

  \item \textbf{remove PCR duplicates for mapped reads on Mbelari}:\\
...
More information on RG tag: \texttt{https://software.broadinstitute.org/gatk/documentation/article.php?id=6472}
  ...
  \item \textbf{create dictionnary for GATK} : \\
...
This was done on the combined fasta to avoid reheader BAM files with removing contigs from coli.
  ...
  \item \textbf{Index input BAM: we sort again after adding RG infos}: \\
\textcolor{blue}{sort\_Mbelari.sh}
...
\textbf{Female reads}:\\
\includegraphics{{../results/mapping/mapped/2017_09_14_female_JU2817_fastqc_per_base_sequence_content_plot_mapped_belari}.png}
\includegraphics{{../results/mapping/mapped/2017_09_14_female_JU2817_fastqc_per_sequence_gc_content_plot_mapped_belari}.png}\\

\textbf{Male reads}:\\
\includegraphics{{../results/mapping/mapped/2017_09_14_male_JU2817_fastqc_per_base_sequence_content_plot_mapped_belari}.png}
\includegraphics{{../results/mapping/mapped/2017_09_14_male_JU2817_fastqc_per_sequence_gc_content_plot_mapped_belari}.png}


% ------------------------------------------------------------
\subsection{FastQC on unmapped (on E.coli and M.belari) reads}

\textcolor{blue}{extract\_unmapped.sh} (PSMN)\\
\textcolor{blue}{bamtofq\_Mbelari.sh}\\

\textcolor{blue}{Need to call FastQC on both unmapped reads, Laurent suggested to add all mapped samples in the multiQC output.}


% ------------------------------------------------------------
\subsection{cdhit on unmapped (on E.coli and M.belari) reads}

\textcolor{blue}{run\_cdhit.sh} (PSMN)\\
...
I subsetted the gff3 for gene regions and converted it in bed format using \textcolor{blue}{convertgff\_to\_bed.sh}. \\
\textcolor{red}{Be careful because it converts 1-based, closed [a, b] GFF3 input into 0-based, half-  open [a-1, b) six-column extended BED so it converts the first coordinate to 0 if it was 1, or 6,037 if it was 6,038.} To correct the coordinates for the genes, I changed the bed with \textcolor{blue}{format\_bed.R}.
...
\includegraphics{{./coverage/number_genes_detected_in_contigs}.pdf}
...
\begin{itemize}
 \item Produce bedpe files: \textcolor{red}{bamtobedpe\_Mbelari.sh}. Eventually, we did not work with these files. (PSMN)\\
  \item Produce bed files with depth at each bp after having sorted and indexed BAM: \textcolor{red}{bamtobed\_Mbelari.sh} (PSMN)\\
 \item Open BAM with \texttt{tablet}: it provides a table with contig name, contig length and number of mapped reads.
\end{itemize}

After rmdup and realignment around INDELs:
\begin{small}
\begin{tabular}{l|l|l}
...
\textcolor{blue}{get\_counts\_per\_gene.sh}\\
It allows to subset bed to genes areas only.
...
\includegraphics{{./coverage/2017_12_14_all_log2_FC}.pdf}\\
\includegraphics{{./coverage/2017_12_14_pval_output_tests_genes_log2_FC}.pdf}
...
% .........................
\subsubsection{Raw variants}

Summaries are in \texttt{results/call\_var/2017\_09\_15\_summary\_raw\_var\_female\_JU2917.txt} and \texttt{results/call\_var/2017\_09\_21\_summary\_realign\_indels\_var\_male\_JU2917.txt} like files.\\
Summaries on intersected files are saved in \texttt{results/call\_var/inter\_female\_male\_realign\_indels/summary\_in\_both\_0002.txt} and \texttt{results/call\_var/inter\_female\_male\_realign\_indels/summary\_in\_MRDR6\_not\_in\_MRDR5\_0000.txt}.
...
\includegraphics{{./call_var/2017_11_09_distance_from_INDEL_to_nearest_variant_female}.pdf}
\includegraphics{{./call_var/2017_11_09_distance_from_INDEL_to_nearest_variant_male}.pdf}\\
\includegraphics{{./call_var/2017_11_09_summary_raw_variants}.pdf}
\includegraphics{{./call_var/2017_11_12_log2_density_ratio_raw_SNPs}.pdf}
...
\includegraphics{{./call_var/2017_12_08_summary_alleles_ALT_between_pools}.pdf}
knowing that only variants sharing the same alt allele will be used for the analysis.\\

\includegraphics{{./call_var/2017_12_13_frequency_SNPs_sexes}.pdf}\\
\includegraphics{{./call_var/2017_12_13_log2_density_ratio_SNPs}.pdf}

% --------------------------------------------------------
\subsection{Make fisher tests and data simulation under H0}

\textcolor{blue}{fisher.R}\\

\includegraphics{{./fisher/2017_12_13_signif_Fisher_contig_gene_level}.pdf}
